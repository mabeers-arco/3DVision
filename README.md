...
The network was able to, in some cases, achieve 3D reconstructions that were visually similar to the actual 3D shape. These successes were achieved despite some clear improvements to the network that could be made. In particular, the network employed was relatively small. A larger network may be able to provide more accuracy for a larger set of objects. Also, the way in which the input data to the network is formatted is not intuitive. Ideally future versions of this network would have a grid of pixels as input, and the presence of edges would be inferred rather than explicitly defined. Also, while the "bad" reconstruction is clearly at a strange angle relative to the true shape, it is still a cuboid object. It is possible that the "bad" reconstruction is actually an acceptable reconstruction with depth reversal, as occurs in the Necker cube illusion. It would be interesting to use optimization to explore this possibility and to provide some measure of how far off a reconstruction is from an optimal reconstruction. The work presented builds upon Mishra and Helie's work in concrete ways. In particular, the introduction of a planarity constraint resulted in more reliable reconstructions of polyhedral objects. However, the constraints of MSDA and planarity generalize poorly to natural objects. Future versions of this work may implement more general constraints such as symmetry and compactness. In summary, the network demonstrates the potential of constraint based approaches for 3D reconstruction within neural networks.
